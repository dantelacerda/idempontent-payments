...
public class DepositController {
    ...
    @PostMapping("/pay")
    public ResponseEntity<String> processPayment(@Valid @RequestHeader("Idempotency-Key") String idempotencyKey, @RequestBody DepositDto paymentRequest) {
        ...
        if (paymentsService.hasBeenProcessed(idempotencyKey) || !paymentsService.validateList(Arrays.asList(paymentRequest))) {
            ...
            paymentsService.processBatchPayments(Constants.DEPOSIT_PAYMENT_TYPE, Arrays.asList(paymentRequest), idempotencyKey);
    ...
    public ResponseEntity<String> processListPayment(@RequestHeader("Idempotency-Key") String idempotencyKey, @RequestBody List<DepositDto> paymentsRequest) {
        ...
        if (paymentsService.hasBeenProcessed(idempotencyKey) || !paymentsService.validateList(convertedList)) {
            ...
            int processed = paymentsService.processBatchPayments(Constants.DEPOSIT_PAYMENT_TYPE, convertedList, idempotencyKey);
